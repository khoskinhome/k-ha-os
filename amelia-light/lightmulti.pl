...
#my $serialised_statty_stuff = $statty_stuff->{${thelight}_lights_state};
my $thelight ="bathroom";
...
for my $i ( 0..99 ){ # I am NEVER going to have 100 lamps in one room

    last if ! exists $statty_stuff->{gpio_conf}{"${thelight}_light_$i"};

    $light_max_num = $i;

    $stylight[$i+1] = ''; #get_style( $statty_stuff->{${thelight}_lights_config}{"${thelight}_light_$i"}{current_state});

    $lightimg[$i+1] = get_light_img( $statty_stuff->{gpio_conf}{"${thelight}_light_$i"}{current_state} );
...
##    $thelight_light_4 => 0,
##    $thelight_light_5 => 1,
##    $thelight_light_6 => "invert",
##    $thelight_lights_all_on => 0,
##    $thelight_lights_all_off => 0,
##    $thelight_lights_all_invert => 0,
##    $thelight_lights_array_invert => [0,3,5],
##    $thelight_lights_array_on => [0,3,5],
##    $thelight_lights_array_invert => [6,7,8],
##
};
#
...
if ( exists $params{all} ) {
    $data_out->{"${thelight}_lights_all_on"} = 1 if $params{state} eq '1';
    $data_out->{"${thelight}_lights_all_off"} = 1 if $params{state} eq '0';
    $data_out->{"${thelight}_lights_all_invert"} = 1 if $params{state} eq 'invert';

#} elsif (  exists $params{disco} ||  exists $params{rand} ||  exists $params{chaser} ){
#    my $type = $params{disco} ;
#    $data_out->{"${thelight}_lights_$type"} = 1;

} else {
    my $light_array = [];
    for my $z ( 0 .. $light_max_num ) {
    ...
    if ( @$light_array && $params{state} eq '1'      ) {
        $data_out->{"${thelight}_lights_array_on"}     = $light_array;
    }
    if ( @$light_array && $params{state} eq '0'      ) {
        $data_out->{"${thelight}_lights_array_off"}    = $light_array;
    }
    if ( @$light_array && $params{state} eq 'invert' ) {
        $data_out->{"${thelight}_lights_array_invert"} = $light_array;
    }
}
...
my $styswitch = get_style( $statty_stuff->{gpio_conf}{"${thelight}_light_switch_detect"}{current_state} );
my $wallswitchimg=get_wall_switch_img($statty_stuff->{gpio_conf}{"${thelight}_light_switch_detect"}{current_state});
